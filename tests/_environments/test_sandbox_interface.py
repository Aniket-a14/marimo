...
class FakeBackend:
    ...
    def add(
        ...
        *,
        ...
        # Both uv and pixi replace a same-named entry when the request
        # carries a constraint, and leave it alone when the request is a
        # bare name.
...
def test_remove_edits_manifest_and_syncs(tmp_path: Path) -> None:
    notebook = tmp_path / "notebook.py"
    notebook.write_text('# /// script\n# dependencies = ["obstore"]\n# ///\n')
    adapter = FakeBackend(tmp_path / "environment")
    sandbox = NotebookSandbox(str(notebook), "pixi", adapter=adapter)
...
def test_rebind_changes_the_source_for_the_next_operation(
    ...
    for notebook in (first, second):
        notebook.write_text("# /// script\n# dependencies = []\n# ///\n")
    adapter = FakeBackend(tmp_path / "environment")
    sandbox = NotebookSandbox(str(first), "pixi", adapter=adapter)

    initial = sandbox.launch(["-m", "example"])
...
def test_unnamed_manifest_persists_on_rebind_and_cleans_up(
    ...
    adapter = FakeBackend(tmp_path / "environment")
    sandbox = NotebookSandbox(None, "pixi", adapter=adapter)
...
def test_close_cleans_unnamed_manifest() -> None:
    sandbox = NotebookSandbox(None, "pixi", adapter=FakeBackend(Path("/env")))
    temporary_source = Path(sandbox.source)
...
def test_pixi_launch_prefers_an_editable_runtime_overlay(
    tmp_path: Path,
) -> None:
    ...
    plan = PixiBackendAdapter().launch(
        environment,
        ["-m", "marimo"],
        overlay=(f"-e {checkout}",),
        base_env={"PATH": "/bin", "PYTHONPATH": "/existing"},
    )

    assert plan.env["PYTHONPATH"] == f"{checkout}{os.pathsep}/existing"
...
def test_pixi_launch_does_not_require_uv(
    ...
    plan = sandbox.launch(["-m", "example"], overlay=("marimo",))

    assert plan.argv[0] == str(root / "bin" / "python")
