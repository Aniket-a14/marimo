    ...
    records = [
        {
            ...
            "kind": "pypi",
            "is_explicit": True,
            # Pixi's list includes inactive extras. They must not define edges.
            "depends": ["polars[pyarrow] ; extra == 'pandas'"],
        },
        {
...
def test_launch_activates_the_conda_prefix(tmp_path: Path) -> None:
    ...
    assert plan.argv == (handle.python, "-m", "marimo")
...
def test_fallback_plan_reflects_this_interpreter() -> None:
    """With no manifest there is no environment manager for pixi; the
    plan runs this interpreter, and inherited activation state must
    describe it rather than an enclosing shell's."""
