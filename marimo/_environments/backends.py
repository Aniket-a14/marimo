...
`UvBackendAdapter` and `PixiBackendAdapter` implement the notebook
sandbox's internal seam; `adapter_for` selects one. The module-level
verbs plan launches for callers that must not edit a manifest (an app
host serving notebooks, the CLI's interpreter-override fallback); they
dispatch through the same adapters, so backend policy lives in one
place.

The managers differ in one contract-visible way: uv layers marimo onto
a launch through the runtime overlay, while pixi has no overlay and
pins marimo into the manifest when the sandbox prepares its source.
Callers pass the overlay unconditionally; the pixi adapter uses it only
to prefer an editable marimo checkout.
...
def launch_fallback(
    ...
    *,
    ...
    uv runs an ephemeral environment (`uv run --isolated`), so packages
    installed during the session die with it. pixi has no ephemeral
    counterpart; the process runs from this interpreter instead.
    """
    if backend == "pixi":
        from marimo._environments.environment import ProcessPlan

        # No environment manager selects the interpreter here, so the
        # inherited activation state must describe this interpreter: a
        # stale VIRTUAL_ENV from an enclosing shell must not leak, and
        # a virtualenv interpreter names its own prefix.
...
class UvBackendAdapter(_ReportingBackendAdapter):
    """uv implementation of the notebook sandbox's internal seam."""
...
class PixiBackendAdapter(_ReportingBackendAdapter):
    """pixi implementation of the notebook sandbox's internal seam."""
    ...
    def launch(
        ...
        *,
        ...
        # pixi has no package overlay. During local development, make the
        # editable marimo checkout supplied by `runtime_overlay` win over the
        # installed runtime dependency without invoking uv or writing a local
        # path into the user's Manifest.
        env = dict(os.environ if base_env is None else base_env)
        editable_paths = [
        ...
        if editable_paths:
            ...
            if existing:
                editable_paths.append(existing)
            env["PYTHONPATH"] = os.pathsep.join(editable_paths)
        return pixi.launch(environment, args, base_env=env)
